...
static void RenderThumbnail(Data *data,FormatRecordPtr fmtRec,PI3DScene *scene,RenderRecordPtr tempRenderRecord) 
{ 
	if (tempRenderRecord == NULL) return;
...
static void DoReadContinue (void)
{
	...
	RenderRecordPtr tempRenderRecord = new RenderRecord;
	if (tempRenderRecord == NULL) 
	{
	...
	uint16 * path = new uint16[2048];
	if (path == NULL) 
	{
		delete tempRenderRecord;
		*gResult=formatCannotRead;
		return;
	}
